...
class PaymentController extends Controller
{
   public function midtrans(Request $request) {
      ...
      $validSignatureKey = hash("sha512", $notification->order_id . $notification->status_code
      . $notification->gross_amount . env('MIDTRANS_API_SERVER_KEY'));
    ...
    if ($type == 'credit_card') {
        if ($fraud == 'challenge') {
            ...
            $message = 'Payment status: ' . $transaction;
            return response(['code' => 200, 'message' => $message], 200);
                
   }

         private function initPaymentGateway() {
        \Midtrans\Config::$serverKey = env('MIDTRANS_API_SERVER_KEY');        
        // Set to Development/Sandbox Environment (default). Set to true for Production Environment (accept real transaction).
        \Midtrans\Config::$isProduction = (bool)env('MIDTRANS_API_PRODUCTION', false);
